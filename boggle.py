...
class Boggle():
    ...
    def make_board(self, xy=5):
        """Make and return a random boggle board."""
        self.xy = xy
        board = []

        for y in range(self.xy):
            row = [choice(string.ascii_uppercase) for i in range(self.xy)]
            board.append(row)
    ...
    def check_valid_word(self, board, word):
        ...
        if word_exists and valid_word:
            print( len(word), word) 
            #!!!!!Gets back tracked? single length character counts for more than 1!!!!!
            self.score += len(word)
            result = "Nice!"
    ...
    def find(self, board, word):
        ...
        for y in range(0, self.xy):
            for x in range(0, self.xy):
